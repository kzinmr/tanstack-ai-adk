...
class Settings(BaseSettings):
    ...
    def set_api_keys_to_env(self) -> "Settings":
        """
        Set API keys to os.environ for ADK-backed model clients.
        """
    ...
    adk_app_name: str = Field(
        default="tanstack_ai_demo",
        description="ADK application name",
    )
    adk_user_id: str = Field(
        default="demo_user",
        description="ADK user id for session scoping",
    )

    # API Keys (optional - ADK clients read these directly from env)
    ...
    database_url: PostgresDsn = Field(
        default="postgresql://postgres:postgres@localhost:54320/tanstack_ai_demo",
    ...
    run_store_backend: str = Field(
        default="memory",
        description="Run store backend (memory)",
    )
