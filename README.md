...
A Human-in-the-Loop (HITL) chat demo using **TanStack AI + ADK (google-adk)**.
...
│       ├── adapters/            # ADK -> TanStack stream adapters
...
- [Google ADK](https://github.com/google/adk-python) - AI agent framework
- [FastAPI](https://fastapi.tiangolo.com/) - Web framework
- [asyncpg](https://github.com/MagicStack/asyncpg) - PostgreSQL driver

### UI Adapter for Agents

- Custom ADK adapter (see `backend/src/backend/adapters/`)

This adapter is the “protocol glue”:

- It parses incoming TanStack AI requests (messages + optional `run_id`).
- It calls ADK `Runner.run_async()` for streaming events.
- It converts ADK events into TanStack AI [StreamChunks](https://tanstack.com/ai/latest/docs/reference/type-aliases/StreamChunk), and emits SSE frames.
- It stores per-run state so a run can pause (approval / client tool) and resume via `/api/continuation`.

**Important internal separation to preserve:**

- **Protocol transformation** (ADK events → TanStack StreamChunks) should remain in the adapter layer.
